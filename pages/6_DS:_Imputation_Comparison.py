...
st.write('The dataset has missing salary information, and the data is missing \
         completely at random (MCAR) as it is missing not because of a relationship\
...
st.write('As can be seen from the figure above, there\'s a lot of missing salary data \
         ...
         is better? That\'s what we will try to answer in the following sections.')
...
st.write('Looking at the violin plot, we can see that there\'s a good amount of salaries below zero \
         ...
         conclusion is that it is not a reliable imputation method for this application. However, \
...
st.write("Comparing the minimum and maximum salaries in the imputed datasets and \
         ...
         is the negative salary values. This confirms to us that we cannot reliably use \
         mean imputation here.")

st.subheader('Thoughts and Conclusion')
st.write('While KNN imputation performed better than mean imputation in this application, there might be \
         more to explore here. Other imputation techniques might yield better results. There could be some \
         useful correlations that can be used to impute salaries here such as the location of the job, \
         the experience level and the required qualifications. Some questions remain unanswered here \
         such as \'Will another imputation method be more reliable? and how to test that?\' \'Is it \
         still a viable solution to impute while having this much missing data?\' \'Is there a better \
         solution? or should we just use the original dataset with missingness?\' Some of these \
         questions are subjective but we need to come up with a way to quantify the results to better \
         answer them.')
st.write("We feel that will need to do more testing using other imputation methods that more factors  and features into \
         ...
         application and may compare it sometimes to the original un-imputed dataset. The reason for this \
